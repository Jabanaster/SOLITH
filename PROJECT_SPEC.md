...
*   **Internal Engine (Win32/Steam/Epic) — ADVANCED TOOL TIER:** Authorized to use in-process C++ DLL injection, Vectored Exception Handling (VEH) hooks, and Hardware Breakpoints (HWBP) for ultimate execution flow interception without corrupting original bytes. This capability is explicitly classified as an **Advanced Tool** — narrowly-scoped, never enabled by default, never part of SOLITH's normal/default architecture, and gated behind explicit user opt-in per target. It is the one named exception to `SOLITH MASTER IMPLEMENTATION PLAN §0.1`'s "must not make part of normal architecture" list for DLL injection/manual mapping/renderer hooks — that rule governs what ships as default/normal behavior across all new and existing subsystems; it does not retroactively revoke this pre-existing, explicitly-authorized Advanced Tool capability. Any other in-process hooking capability proposed in the future must clear this same Advanced Tool bar (narrow scope, opt-in, explicitly authorized here) before being added — it may never be added silently as "normal" architecture.
