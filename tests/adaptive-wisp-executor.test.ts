...
function fakeAdapter(options: FakeAdapterOptions = {}) {
  ...
  const adapter: WispTrainerExecutionAdapter = {
    ...
    confirmFreeze: (proposalId, token) => {
      calls.confirmFreeze++;
...
describe('executeWispAction — toggle', () => {
  test('OFF -> ON calls proposeWrite with true, then confirms as enabled', () => {
    ...
    const result = executeWispAction(request, actionDef(), alphaBinding(), alphaContext(), deps(adapter));
    assert.equal(result.ok, true);
    assert.equal(result.status, 'enabled');
    assert.equal(calls.proposeWrite, 1);
  });

  test('ON -> OFF calls proposeWrite with false, then confirms as disabled', () => {
  ...
  test('an entry that does not support toggle is rejected as unsupported — no raw writer called', () => {
...
describe('executeWispAction — set (Section 43)', () => {
  ...
  test('a valid integer reaches canonical execution', () => {
  ...
  test('NaN is rejected before reaching canonical execution', () => {
  ...
  test('a value out of int32 range is rejected', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
  ...
  test('a wrong-type value (string for an int32 entry) is rejected', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
  ...
  test('a boolean value is valid for a bool-typed entry', () => {
...
describe('executeWispAction — increment (Section 44)', () => {
  ...
  test('current 100 + preset +10 = 110 through the canonical path', () => {
    const { adapter } = fakeAdapter({
      states: supports,
      proposeWrite: (_g, _e, value) => {
    ...
    const result = executeWispAction(request, def, alphaBinding(), alphaContext(), deps(adapter));
    assert.equal(result.ok, true);
  });

  test('an increment that would overflow int32 range is rejected', () => {
    ...
    const result = executeWispAction(request, overflowDef, alphaBinding(), alphaContext(), deps(adapter));
    assert.equal(result.status, 'rejected');
    assert.equal(result.diagnostic?.code, 'WISP_EXECUTION_INVALID_VALUE');
    assert.equal(calls.proposeWrite, 0);
  });

  test('an unknown presetId is rejected as an invalid preset', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
  ...
  test('a stale session yields zero canonical execution calls', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
    const request: WispActionExecutionRequest = { control: 'increment', actionId: 'a-health', profileId: 'p1', presetId: 'plus-ten', consentToken: 'tok' };
    const result = executeWispAction(request, def, alphaBinding({ sessionGeneration: 10 }), alphaContext({ sessionGeneration: 11 }), deps(adapter));
...
describe('executeWispAction — multiplier (Section 45)', () => {
  ...
  test('an allowed preset routes through the canonical path', () => {
    const { adapter } = fakeAdapter({
      states: supports,
      proposeWrite: (_g, _e, value) => {
  ...
  test('a structurally valid preset that the current entry does not support multiplier for is rejected before any preset lookup', () => {
...
describe('executeWispAction — cycle (Section 46)', () => {
  ...
  test('current 1x -> 2x, current 2x -> 5x, current 5x wraps to 1x (documented wrap policy)', () => {
    let seen: unknown;
    ...
    executeWispAction({ control: 'cycle', actionId: 'a-health', profileId: 'p1' }, def, alphaBinding(), alphaContext(), deps(a1));
    assert.equal(seen, 2);
    ...
    executeWispAction({ control: 'cycle', actionId: 'a-health', profileId: 'p1' }, def, alphaBinding(), alphaContext(), deps(a2));
    assert.equal(seen, 5);
    ...
    executeWispAction({ control: 'cycle', actionId: 'a-health', profileId: 'p1' }, def, alphaBinding(), alphaContext(), deps(a3));
    assert.equal(seen, 1);
  });
});

describe('executeWispAction — momentary (Section 47)', () => {
  test('a momentary action with a valid preset invokes the canonical write path', () => {
  ...
  test('momentary without a presetId is rejected — it cannot become a generic execute-callback field', () => {
    const def = actionDef({ presets: [{ id: 'heal', label: 'Heal', value: 100 }] });
    const { adapter, calls } = fakeAdapter({ states: { 'game-alpha:health': { dataType: 'int32', supportsControls: ['momentary'] } } });
  ...
  test('an entry with no canonical momentary support reports unsupported rather than inventing a privileged implementation (Section 66)', () => {
    const def = actionDef({ presets: [{ id: 'heal', label: 'Heal', value: 100 }] });
...
describe('executeWispAction — freeze (Section 42)', () => {
  ...
  test('freeze enable invokes the existing freeze adapter exactly once', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
  ...
  test('freeze disable routes through the canonical unfreeze path, no consent phase', () => {
  ...
  test('a stale binding results in zero freeze calls', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
    const request: WispActionExecutionRequest = { control: 'freeze', actionId: 'a-health', profileId: 'p1', enable: true, consentToken: 'tok' };
    const result = executeWispAction(request, actionDef(), alphaBinding({ sessionGeneration: 10 }), alphaContext({ sessionGeneration: 11 }), deps(adapter));
    assert.equal(result.status, 'stale');
    assert.equal(calls.proposeFreeze, 0);
...
describe('executeWispAction — consent (Section 54)', () => {
  ...
  test('a write requiring consent does not write before approval — result is pending-consent, confirmWrite never called', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
  ...
  test('a rejected consent produces no write and a rejected status', () => {
  ...
  test('an expired consent token (adapter reports rejected) produces no write', () => {
  ...
  test('a replayed consent token (adapter reports rejected) produces no write', () => {
  ...
  test('a session change while consent is pending is rejected before the executor ever calls confirmWrite (Section 23, 36)', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
...
describe('executeWispAction — security: stale/cross-game/table-trainer/detached (Section 48-51)', () => {
  ...
  test('cross-game binding cannot execute — current game is Beta, binding is Alpha', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
    const request: WispActionExecutionRequest = { control: 'set', actionId: 'a-health', profileId: 'p1', value: 99, consentToken: 'tok' };
    const result = executeWispAction(request, actionDef(), alphaBinding(), alphaContext({ gameId: 'game-beta' }), deps(adapter));
    assert.equal(result.status, 'stale');
    assert.equal(calls.proposeWrite, 0);
  });

  test('cross-table binding cannot execute', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
    const request: WispActionExecutionRequest = { control: 'set', actionId: 'a-health', profileId: 'p1', value: 99, consentToken: 'tok' };
    const binding = alphaBinding({ tableId: 'table-a' });
    const result = executeWispAction(request, actionDef(), binding, alphaContext({ tableId: 'table-b' }), deps(adapter));
    assert.equal(result.status, 'stale');
    assert.equal(calls.proposeWrite, 0);
  });

  test('a detached (null) context always rejects execution', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
    const request: WispActionExecutionRequest = { control: 'set', actionId: 'a-health', profileId: 'p1', value: 99, consentToken: 'tok' };
    const result = executeWispAction(request, actionDef(), alphaBinding(), null, deps(adapter));
    assert.equal(result.status, 'stale');
    assert.equal(calls.proposeWrite, 0);
  });

  test('a missing identity-bridge mapping rejects execution before entry lookup', () => {
    const noBridge = createExplicitGameIdentityBridge({});
    const { adapter, calls } = fakeAdapter({ states: supports });
    const request: WispActionExecutionRequest = { control: 'set', actionId: 'a-health', profileId: 'p1', value: 99, consentToken: 'tok' };
    const result = executeWispAction(request, actionDef(), alphaBinding(), alphaContext(), { entryLookup: lookup, identityBridge: noBridge, trainerAdapter: adapter });
    assert.equal(result.diagnostic?.code, 'WISP_EXECUTION_IDENTITY_MAPPING_MISSING');
    assert.equal(calls.proposeWrite, 0);
  });

  test('a raw address field on the request is rejected before any binding/entry work happens', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
    ...
    const result = executeWispAction(hostile, actionDef(), alphaBinding(), alphaContext(), deps(adapter));
    assert.equal(result.diagnostic?.code, 'WISP_EXECUTION_REQUEST_SHAPE_REJECTED');
    assert.equal(calls.proposeWrite, 0);
  });

  test('a pid field on the request is rejected', () => {
    const { adapter, calls } = fakeAdapter({ states: supports });
  ...
  test('a missing/disabled entry is rejected as unavailable, not a crash', () => {
    const disabledLookup = fixtureLookup({ 'game-alpha:health': entryDescriptor('health', false) });
    const { adapter, calls } = fakeAdapter({ states: supports });
    const request: WispActionExecutionRequest = { control: 'set', actionId: 'a-health', profileId: 'p1', value: 99, consentToken: 'tok' };
    const result = executeWispAction(request, actionDef(), alphaBinding(), alphaContext(), { entryLookup: disabledLookup, identityBridge: bridge, trainerAdapter: adapter });
    assert.equal(result.status, 'unavailable');
