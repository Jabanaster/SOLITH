...
| Pointer maps (model + live orchestration) | PARTIAL | P2-1/P2-2 implemented and tested (522/522 live-memory, 2033/2033 root); real spawned-process proof, not a shipped-title game. P2-3.1 closed pointer-chain visualization and real cancellation. P2-4 closed the pointer-stability classification model, persistence, and UI (see the pointer-stability-testing row below) with a real 10/10 fixture restart campaign, but real-game restart evidence remains incomplete — a safe, zero-gameplay-scripting ground truth could not be found against either of two real installed titles tried (`Docs/phase2/018`). The combined pointer-maps capability stays PARTIAL, not Phase-2 certified, pending that real-game evidence | 2 |
...
| Pointer stability testing | PARTIAL | P2-4: real classification model (`Docs/phase2/016`) distinguishing `stable_exact`/`stable_relocated`/`target_moved_chain_valid`/`chain_broken`/`module_missing`/`read_failed`/`process_exited`/`false_positive`, with independent ground-truth verification (never "resolved to readable memory" alone). A real 10-restart fixture campaign (`Docs/phase2/017`) proved 10/10 correct classifications against a genuinely killed-and-relaunched native process, including real heap relocation on 9/9 restarts. Persistence migrated schemaVersion 1→2 with real backward compatibility for pre-P2-4 saved maps (`Docs/phase2/019`). UI extension (`Docs/phase2/019`) and real-process/real-UI proof (`Docs/phase2/019`) both pass. NOT complete: real-game restart evidence — two real installed titles (Godlike Burger, Bastion) were attached to read-only with zero writes, but a safe, automatable, zero-gameplay-scripting ground truth could not be found within reasonable effort against either (`Docs/phase2/018`), so no real-game restart classification exists yet. A pre-existing (not P2-4-introduced) defect in the pointer-map Load workflow was also found and filed separately, not fixed in this stage (`Docs/phase2/020`) | 2 |
...
- Current checkpoint: **P2-4 — Pointer Stability / Real-Game Restart Validation (IN PROGRESS — model/fixture-campaign complete, real-game evidence open)**
...
- **P2-A — Pointer Engineering & Stability** — IN PROGRESS
  ...
  - P2-4 Pointer stability / real-game restart validation — IN PROGRESS (classification model, fixture restart campaign, persistence, UI: complete; real-game restart evidence: open)
