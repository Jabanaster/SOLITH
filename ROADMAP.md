...
### Active offline product blockers (do not mark done without proof)

These are current near-term blockers and must remain separate from live-game validation work and from partially certified Wisp work.

1. **Trainer Library manual metadata persistence** - the manual add/edit flow must durably persist cover, icon, save locations, notes, executable path, and metadata ID. Required proof: create, reload, edit-one-field, clear-one-field, migrate old core-only rows, restart persistence, and remove metadata without deleting referenced game files.
2. **CT ZIP import bridge and preview gate** - CT ZIP import must use the Electron desktop file-picker bridge, show explicit progress, generate a preview before persistence, reject malformed/unsafe archives clearly, support cancel/retry, and confirm that declined previews write no catalog records. Browser `File.path` fallback is not acceptable.
3. **Page-level walkthrough help** - every major module page needs a consistent help/walkthrough affordance explaining what the page does, what is metadata-only, what is read-only, what can write files, and which safety gates apply. This is product guidance, not hidden documentation.
4. **Advanced Scan process picker filtering** - process selection must default to likely game targets, deny critical/system processes, keep unknown processes behind an explicit "show all" mode, and display confidence and reason codes before any attach.
5. **Advanced Scan process picker sorting** - default sort is A-Z by game title, with exact current-game match pinned first. Groups: Current game, Installed games, Likely games, Unknown processes only when enabled. Visible sort controls: A-Z, Z-A, Confidence, Recently detected, PID. Refresh/search/filter must preserve the active sort.
6. **Wisp in-app and Electron overlay fallback certification** - Wisp remains uncommitted until manual certification passes. Known blockers: bottom-edge expanded controls, expanded desktop-overlay click-through hull, expanded desktop-overlay movement constraints, alternate transformations, and the runtime failure where Wisp disappears after a game starts or after clicking it. The Electron overlay is now classified as desktop/windowed-game fallback infrastructure, not the final WeMod-style in-game host.
7. **Wisp runtime disappearance investigation** - reproduce with event tracing for overlay visibility, bounds, always-on-top, focus, ignore-mouse-events, expanded/collapsed state, display ID, renderer crash/unresponsive events, and IPC errors. Click/open controls must be separate from hide/close. Game focus and display-mode changes must preserve visibility or surface an explicit limitation.
8. **Wisp Game Bar overlay host prototype** - implement a minimal Xbox Game Bar widget proof of concept as the preferred in-game Wisp host. Required proof: Wisp visible over a real game, pin/unpin works, game retains focus, click-through behaves correctly, visible controls remain clickable, Win+G lifecycle works, minimize/restore and alt-tab are stable, multiple monitor/DPI behavior is acceptable, and state synchronizes with Solith over an authenticated local channel.
9. **Wisp overlay architecture comparison** - compare Xbox Game Bar widget, native Windows overlay process, current Electron BrowserWindow overlay, and direct graphics injection. Score fullscreen compatibility, borderless compatibility, click-through accuracy, anti-cheat risk, implementation cost, maintenance cost, crash isolation, accessibility, and deployment complexity. Default decision: Electron remains the main app and fallback overlay; Xbox Game Bar is the primary in-game host; native overlay only if Game Bar cannot satisfy required behavior; direct graphics injection is not a general Solith solution.
