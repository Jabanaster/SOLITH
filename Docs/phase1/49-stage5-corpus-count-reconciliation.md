...
**Stage 5.4 update (owner decision #3):** the proportional ≈4,714/≈4,064 figures previously shown in this table were forensic arithmetic only (972 files × this corpus's own average AOB/file rate, and the arithmetic remainder) — never independently verified against real per-signature evidence. Stage 5.4's owner decision explicitly forbids using them in any certification calculation, and prefers they not appear in a certification-facing summary at all. They are kept below **only** as a labeled historical forensic note, struck from every place they could be mistaken for a certified count.

| Reason | Count | Confidence |
|---|---|---|
| Raw-file-count difference (23,953 vs 22,981 = 972 files not present in this session's fresh count of the same archive) | 972 files. **[NON-AUTHORITATIVE FORENSIC ESTIMATE, not used in any certification calculation per Stage 5.4 owner decision #3]**: ≈4,714 AOB signatures at this corpus's own measured 4.85 AOB/file average. | The file-count difference itself is real and code-confirmed; the AOB-count figure derived from it is an unverified proportional guess, not evidence — the mechanism producing the file-count difference could not be identified (see below) |
...
| Other / genuinely unattributed remainder | **[NON-AUTHORITATIVE FORENSIC ESTIMATE, not used in any certification calculation]**: ≈4,064 (8,778 − the ≈4,714 estimate above, itself unverified) | Unresolved — Stage 5.4 treats the full 8,778 gap as unattributed, not partially explained by this arithmetic |
