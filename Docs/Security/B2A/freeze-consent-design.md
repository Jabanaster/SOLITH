...
ID. PID, identity, and renderer ID are reconstructed by the main process.
Address/data type/value mismatches return `target_mismatch`; PID or process
identity mismatches return `pid_mismatch`. Consent expires after 60 seconds.
...
are written through `MemoryAuditLog`. Token IDs in audit data use the
first-four-characters plus `…redacted` representation. Raw token material is
returned only to the approving renderer and is never included in previews or
audit records.
