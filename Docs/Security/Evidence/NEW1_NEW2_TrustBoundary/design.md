...
Every check calls the existing `validateIpcSender()` (electron/sender-validation.ts) → `validateTrustedSender()` (src/core/security/trusted-sender-registry.ts) — the same registry and function already used by the freeze/rollback path. No new or parallel trust mechanism was created. All 16 destructive channels in the NEW-1 subsystem (V2 live-memory / injector-launch / in-process-hook / TrainerHost) now share this exact mechanism — 6 already hardened before this branch (freeze/rollback), 10 hardened across this branch's two implementation passes — see `remaining-risks.md`'s "Final same-class destructive-handler audit" for the complete classification of every `ipcMain.handle` in both files, confirming zero `REMAINING DEFECT` entries within that subsystem. A second, narrowly-scoped independent review of exactly the 3 handlers added in the second corrective pass (`in-process-confirm-hook`, `in-process-rollback-hook`, `trainer-host-rollback`) returned `VERIFIED WITH CONDITIONS`; its one substantive condition was an evidence-accuracy overclaim (corrected in `remaining-risks.md`), and it mutation-tested all 3 handlers by deleting the check and confirming the negative-control test fails — see `independent-review.md`.
