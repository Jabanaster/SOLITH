...
**Finding SOL0-P0-1 (CRITICAL — P0, reported per governing rule, NOT fixed this session):**
`electron/privileged-consent-dialog.ts:45-53` (`resolveDialogImpl`) and `:122` (`requestPrivilegedWriteConsent`) read `SOLITH_PRIVILEGED_CONSENT` / `SOLITH_CONSENT_TTL_MS` from `process.env` with **no `app.isPackaged` gate whatsoever** — unlike the established pattern elsewhere in the codebase (`electron/runtime-trust.ts`'s `isDevRuntime()` correctly gates all dev/test overrides on `!app.isPackaged`). Setting `SOLITH_PRIVILEGED_CONSENT=auto-approve` in the environment before launching the packaged app **silently auto-approves every privileged consent dialog** — the human-in-the-loop gate for live-memory write confirmation, freeze start, and injector-helper launch — with no user interaction, no dialog shown, and (per the emergency-stop/audit-logging findings) the bypass itself is not separately logged. `SOLITH_CONSENT_TTL_MS` similarly lets an attacker with env-set capability extend consent token lifetime arbitrarily.
Per the governing rule ("If a critical unsafe path exists: STOP before implementing a behavioral change and report it as P0"), this is reported, not patched, in this SOL-0 pass. **This must be the first item addressed before or alongside SOL-1**, ideally as its own tightly-scoped P0 fix (add `!app.isPackaged &&` to both checks, mirroring `runtime-trust.ts`'s existing pattern) rather than folded into SOL-1's broader authority work.
...
| G2 | `SOLITH_PRIVILEGED_CONSENT`/`SOLITH_CONSENT_TTL_MS` env overrides lack packaged-build gate | **Critical (P0, SOL0-P0-1)** | Gate on `!app.isPackaged`, mirroring `runtime-trust.ts` | `live_memory_confirm_write`, `live_memory_freeze_start`, `injector_confirm_launch` | Low — 2-line fix, isolated file |
...
**SOL-0 = PARTIAL / NOT CERTIFIED.**

Per the governing exit gate (Step 17), every checklist item is substantively complete (inventory, taxonomy, mapping, authority model, consent model, destructive policy, machine scope, control-provider inventory, emergency-stop behavior, process lifecycle, browser-control absence proven, audit/postcondition coverage, action authority matrix, SOL-1 gap list, focused tests green, full regression green) **except one**: *"no undocumented P0 authority vulnerability remains."* SOL0-P0-1 is documented, not fixed — by design, per the governing rule that SOL-0 must STOP and report rather than implement a fix. That is precisely why full CERTIFIED status cannot be claimed this pass: the gate requires the P0 be resolved, and resolving it is explicitly out of SOL-0's scope.

**This is not a downgrade for its own sake** — every other exit-gate item is genuinely met with reproducible evidence above.
