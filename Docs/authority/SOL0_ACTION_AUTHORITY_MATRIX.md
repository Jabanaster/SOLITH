...
| ~~G7~~ | ~~`readOnlyMode` kill switch fully built but unreachable in production~~ | **RESOLVED 2026-09-01 (SOL-1)** | Wired to `authority-set-readonly` IPC / `electron/authority-bridge.ts`; `AuthorityService.evaluate()` denies every mutating capability when set | Global write gate | Closed |
...
| ~~G9~~ | ~~`revoking` flag checked once before `await`, not re-checked after~~ | **RESOLVED 2026-09-01 (SOL-1)** | Explicit `this.revoking` re-check added immediately before each memory write in `confirmWrite`/`rollback`/freeze-tick, after their respective `await` gaps | `memory.write`, `memory.rollback`, freeze | Closed |
...
| ~~G11~~ | ~~TrainerHost process termination is tool-return-only in production; PID-death only verified in test harness~~ | **RESOLVED 2026-09-01 (SOL-1)** | `stop()` now calls the existing `verifyExitOrKill()` probe (`process.kill(pid,0)`, force-terminates if still alive) instead of assuming shutdown/SIGTERM succeeded | `process.kill` | Closed |
...
G2 in the SOL-1 gap list is closed accordingly. As of the SOL-1 pass on 2026-09-01, G7, G9, and G11 are also closed (see rows above and `Docs/authority/SOL1_GOVERNED_COMPUTER_CONTROL.md`). G1, G3, G4, G5, G6, G8, G10 remain open — SOL-1 is PARTIAL, not CERTIFIED; see the SOL-1 evidence doc for exactly what's implemented versus outstanding.
