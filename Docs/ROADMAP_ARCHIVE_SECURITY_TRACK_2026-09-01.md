# SOLITH MASTER ROADMAP (ARCHIVED — security/authority track, superseded 2026-09-01)

> **ARCHIVED.** This was `MASTER_ROADMAP.md` through 2026-09-01. It is
> superseded by the gaming-first roadmap in `MASTER_ROADMAP.md`, which
> redefines SOL-0 through SOL-22 around gaming intelligence rather than
> machine-authority infrastructure. Preserved verbatim as historical
> evidence — do not treat its SOL-0/SOL-1 status lines as current. The
> authority-layer work this track produced (SOL0-P0-1 fix, the SOL-1
> `AuthorityService` evaluator) still exists in the codebase
> (`src/core/authority/`, `Docs/authority/`) and is pending classification
> under the new roadmap's SOL-0 audit (PRESERVE / ADAPT / MOVE-EXTRACT /
> REMOVE / DEFER) — see the new `MASTER_ROADMAP.md` §"Pending reconciliation."
>
