...
| MP-P0.2 | Uninstall data protection (`deleteAppDataOnUninstall: false`, disposable/durable split, uninstall + reinstall recovery tests) | **PARTIAL** — `package.json`'s `build.nsis.deleteAppDataOnUninstall` flipped to `false`. Disposable/durable storage split and the uninstall/reinstall recovery tests themselves (MP-P0.3) remain **PENDING** |
| MP-P0.3 | Recovery storage split (disposable cache/thumbnails/scans/logs vs. durable Recovery Ledger/receipts/backup ownership/trusted catalog/definitions/research metadata) | **PARTIAL — MECHANISM IMPLEMENTED, ONE CONSUMER MIGRATED** — `src/shared/storage-classes.ts`'s `reconcileStorageClasses()` creates `disposable/`/`durable/` roots and idempotently migrates legacy directories via a durable, crash-resumable marker (7 tests: fresh-install, migration, no-op, idempotent re-run, interrupted-migration resume, corrupt-marker fallback, durable-survives-disposable-wipe). Only `logs/` is migrated so far (3 write sites updated). **Remaining**: `backups/`, `solith.db`, `research-sessions/` still live at their original flat paths — each needs its own consumer audit (`src/core/saves/editor.ts`, `src/core/database/index.ts`, `electron/live-memory-ipc.ts`, `electron/avowed-wingdk-backup-watch.ts`) before moving; no explicit permissions-failure test |
