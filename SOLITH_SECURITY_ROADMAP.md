...
| MP-P0.5 | Handle-based path authorization (canonical handles, volume/file identity, reparse checks, symlink/junction/mount-point/hardlink/rename-race defenses) | **PARTIAL — CORE IMPLEMENTED, TWO CONSUMERS MIGRATED** — `src/core/safety/handle-path-authorization.ts` opens an OS handle, walks every path component for reparse points (not just the leaf), and binds identity to fstat's dev/ino from that handle; `reauthorizeBeforeCommit()` revalidates immediately before mutation. A latent fd leak on the reparse-on-canonical/containment-failure/catch-all failure paths (present since the original P0.5 commit) was found and fixed during the P0.4 pass. Wired into `src/core/safety/atomic-write.ts` and, as of P0.4, `src/core/safety/mutation-transaction-service.ts`. 15 adversarial tests pass (`tests/handle-path-authorization.test.ts`), including live symlink/junction escapes, rename-after-auth, replace-after-auth, and identity-mismatch races; mount-point coverage is documented as untestable in this environment (no second volume) and relies on the same component-wise reparse scan as junctions. **Remaining**: `path-safety.ts`'s string-based `validatePathSafety`/`isContainedWithin`/`getCanonicalPath` is still used directly by 10 files / 31 call sites (verified via grep, not estimated — the earlier "~57" figure in this document was unverified and is corrected here): `src/core/backups/index.ts` (6), `src/core/saves/locations.ts` (8), `src/core/safety/file-lock.ts` (3, non-security coordination-key use), `electron/main.ts` (2), `electron/ipc-validation.ts` (2), `src/core/ct-library/ct-zip-picker-bridge.ts` (1), `src/core/pilot/intake.ts` (1), `src/core/safety/atomic-write.ts` (1, superseded by its own P0.5 authorization call but the import remains for `getCanonicalPath`'s lock-key use). None of these 10 files have been individually classified or migrated yet. Hardlink defense not explicitly covered. |
