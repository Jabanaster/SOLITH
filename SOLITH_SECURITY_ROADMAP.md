...
| # | Item | Status |
|---|---|---|
| MP-P0.1 | Packaged-runtime boundary (no `--dev`/`SOLITH_DEV`/dev-server routing/test overrides in production; unsafe `ELECTRON_USER_DATA_PATH` behavior removed) | **DONE (source-level)** — `electron/runtime-trust.ts`'s `isDevRuntime()`/`isCompatTestRuntime()` gate every override on `!app.isPackaged`; `main.ts`/`trainer-overlay.ts`/`wisp-overlay.ts` all route through it; `ELECTRON_USER_DATA_PATH` gated the same way. Static-boundary-enforced by `tests/packaged-runtime-boundary-static.test.ts`. **Remaining for MP-P0.12**: a real packaged-installer-level adversarial proof (build the NSIS installer, launch the installed EXE with `--dev`, confirm it refuses) — not yet performed |
| MP-P0.2 | Uninstall data protection (`deleteAppDataOnUninstall: false`, disposable/durable split, uninstall + reinstall recovery tests) | **IMPLEMENTED — UNCERTIFIED** — `build.nsis.deleteAppDataOnUninstall:false` verified. `tests/uninstall-recovery.test.ts` (7 tests) provides deterministic integration-level evidence for uninstall/reinstall/upgrade/missing-root/corrupt-and-future-schema-marker scenarios, built on top of MP-P0.3's now-complete storage split. Migration-state marker gained explicit `schemaVersion`. **Remaining**: the real NSIS installer/uninstaller has never been executed end-to-end in this environment — explicitly documented as out of scope here, not silently skipped; that gap is why this is UNCERTIFIED rather than CERTIFIED |
| MP-P0.3 | Recovery storage split (disposable cache/thumbnails/scans/logs vs. durable Recovery Ledger/receipts/backup ownership/trusted catalog/definitions/research metadata) | **ALL NAMED CONSUMERS MIGRATED — UNCERTIFIED PENDING FULL-CAMPAIGN HOSTILE REVIEW** — `src/shared/storage-classes.ts`'s `reconcileStorageClasses()` creates `disposable/`/`durable/` roots and idempotently migrates legacy paths (now both directories and single files) via a durable, schema-versioned, crash-resumable marker. `logs/` → disposable; `backups/` (`src/core/saves/editor.ts`, `electron/avowed-wingdk-backup-watch.ts`), `research-sessions/` (`electron/live-memory-ipc.ts`), and `solith.db` (`src/shared/app-paths.ts`'s `resolveDatabasePath()`) → durable. 9 storage-classes tests + 7 uninstall-recovery tests + 3 static consumer-migration tests, all passing, plus full regression (npm test, test:v1+SQL, build:electron, electron-smoke — the last of which boots the real app and reads the DB through the new durable path). **Note on `backups/`**: existing backup files/DB rows at the pre-migration location are intentionally left in place (new-writes-only migration) since each backup record's absolute path is captured at creation time — this is a deliberate, safe design choice, not incomplete work. No remaining named P0.3 consumer migration debt. |
| MP-P0.4 | `MutationTransactionService` — one authoritative save/file mutation pipeline (authorization, identity binding, durable backup, durable journal, temp write/validate, pre-commit revalidation, atomic replace, final verification, durable receipt, startup recovery classification); no save provider writes directly to a path | **MUTATION TRANSACTION CORE — IMPLEMENTED AND TESTED, ONE PROVIDER MIGRATED** — `src/core/safety/mutation-transaction-service.ts` implements the full pipeline as a self-contained parallel path (does not wrap `atomic-write.ts`; consolidating the two is future work, not done). Durable backups/journal/receipts live under `<durableRoot>/recovery/{backups,transactions,receipts}/`. 27 tests (`tests/mutation-transaction-service.test.ts`) across success, authorization (outside-root, junction escape, deleted-then-recreated target), concurrency (same-identity rejection, lock release), backup (write/verify failure), temp (write/roundtrip-validation failure), replace (pre-replace failure vs. post-replace failure — the latter correctly forces `RECOVERY_REQUIRED`, never `ABORTED`), journal (write failure, malformed/incomplete record → `CORRUPT_RECORD`), recovery classification (`SAFE_ABORT`/`RECOVERY_REQUIRED`/`ALREADY_COMMITTED`/idempotent re-run), and cleanup. Not every one of the plan's 40 enumerated scenarios has a distinct test — several collapse onto one code path (documented in the P0.4 commit message), not silently dropped. First provider migrated: `writeJsonSaveField` (`src/core/saves/json-save-field.ts`) — external contract (params, return shape, thrown error strings) unchanged; still deposits a legacy `${filePath}.trainer-backup` sibling copy so `write-save-field.ts`'s separate `rollbackWriteField()` path keeps working unmodified. `host-supervisor.ts`'s child process now receives the real durable root via `SOLITH_DURABLE_ROOT` env, resolved by the caller (an already-async IPC handler) rather than inside `start()` (which must stay synchronous-up-to-spawn). **Remaining**: only 1 of the ~24 discovered mutation writers has been migrated; `ini-save-field.ts`, `binary-save-field.ts`, `write-save-field.ts`'s XML/backup paths, `pilot/intake.ts`, `backups/index.ts`, `backups/avowed-wingdk.ts` are NOT migrated and still use their own ad hoc backup+rename logic; a full writer classification table (per plan §3) has not been written up as a standalone artifact. |
| MP-P0.5 | Handle-based path authorization (canonical handles, volume/file identity, reparse checks, symlink/junction/mount-point/hardlink/rename-race defenses) | **PARTIAL — CORE IMPLEMENTED, THREE CONSUMERS MIGRATED, REMAINING 9 CLASSIFIED** — `src/core/safety/handle-path-authorization.ts` opens an OS handle, walks every path component for reparse points (not just the leaf), and binds identity to fstat's dev/ino from that handle; `reauthorizeBeforeCommit()` revalidates immediately before mutation. A latent fd leak (found during the P0.4 pass) was fixed. Wired into `src/core/safety/atomic-write.ts`, `src/core/safety/mutation-transaction-service.ts` (P0.4), and now `src/core/backups/index.ts`'s `restoreBackup()` (a genuine TOCTOU close — the prior lexical containment check ran once at the top of the function with several statements, including a temp-file copy, between it and the final rename). 16 adversarial + 1 static consumer-migration test pass; mount-point coverage remains documented-untestable (no second volume in this environment), hardlink defense not explicitly covered. **Remaining 9 files / ~25 call sites, classified this pass** (not left as an undifferentiated count): `src/core/saves/locations.ts` (8 call sites) — CONFIGURATION/APPROVAL_METADATA, probes only (`existsSync`/`statSync`), never writes; the real security boundary is write-time re-authorization, already covered by the three migrated writers above — defense-in-depth candidate, not urgent. `src/core/pilot/intake.ts` (1) — LOW priority: copies FROM an arbitrary user-picked source INTO a freshly-created unique workspace path every call, so there is no destination-side TOCTOU window to close (unlike the write-to-existing-file pattern the other migrations address); already re-hashes its source before/after copy as its own preimage check. `src/core/ct-library/ct-zip-picker-bridge.ts` (1) — READ_ONLY, validates a user-picked import path before reading it. `electron/ipc-validation.ts` (2), `electron/main.ts` (2) — not yet individually inspected this pass. `src/core/safety/file-lock.ts` (3) — non-security lock-key normalizer (in-memory mutex coordination), not an authorization boundary, out of scope for migration. `src/core/safety/atomic-write.ts` (1) — superseded by its own P0.5 authorization call; the `getCanonicalPath` import remains only for the file-lock key. |
| MP-P0.6 | Optimistic concurrency (file ID/volume ID/hash/size/generation revalidated immediately before commit; abort on drift) | **PARTIAL — WHOLE-FILE PREIMAGE HASH NOW ENFORCED IN BOTH PIPELINES, NO SESSION-LEVEL MODEL** — Building this found a real gap: both `mutation-transaction-service.ts`'s and `atomic-write.ts`'s pre-commit revalidation checked file identity (dev/ino) only, not content. A same-inode in-place rewrite by an external process (identical identity, different bytes) would pass identity revalidation and get silently overwritten. Both now also re-hash the live target against the originally captured preimage hash immediately before the mutating rename, and abort safely (replace hasn't happened yet) on mismatch (`mutation-transaction-service.ts` commit `e6e82aa`, new test 9b proving the identity-only check was insufficient; `atomic-write.ts` commit `c20f343`, full regression still green: test:v1 246/246, full test 1268/1268, build:electron 29/29). `atomic-write.ts`'s `expectedOldValue` single-field compare is unchanged and still scoped to whichever field the caller supplies. **Remaining**: no session-level (cross-call) generation/staleness model exists yet — each transaction only defends its own single-call window, not a UI read-then-later-approve flow spanning multiple calls; that's still open. |
