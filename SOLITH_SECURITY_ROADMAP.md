...
| MP-P0.5 | Handle-based path authorization (canonical handles, volume/file identity, reparse checks, symlink/junction/mount-point/hardlink/rename-race defenses) | **PARTIAL — CORE IMPLEMENTED, NOT FULLY ADOPTED** — `src/core/safety/handle-path-authorization.ts` opens an OS handle, walks every path component for reparse points (not just the leaf), and binds identity to fstat's dev/ino from that handle; `reauthorizeBeforeCommit()` revalidates immediately before mutation. Wired into `src/core/safety/atomic-write.ts` in place of the old string-based check. 15 adversarial tests pass (`tests/handle-path-authorization.test.ts`), including live symlink/junction escapes, rename-after-auth, replace-after-auth, and identity-mismatch races; mount-point coverage is documented as untestable in this environment (no second volume) and relies on the same component-wise reparse scan as junctions. **Remaining**: `path-safety.ts`'s string-based `validatePathSafety`/`isContainedWithin` is still used by every *other* call site (saves, backups, definitions import, trainer-research, etc. — see the 57-file grep hit list) and has not been audited/migrated; hardlink defense not explicitly covered |
