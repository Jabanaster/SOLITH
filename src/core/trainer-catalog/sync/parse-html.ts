...
import { isPlaceholderTitle } from './placeholder-titles';
...
export function parseTrainerListHtml(baseUrl: string, html: string): ParsedRemoteTrainer[] {
  ...
  while ((match = titleRegex.exec(html)) !== null) {
    ...
    if (!gameName || gameName.length < 2 || isPlaceholderTitle(gameName)) continue;
...
export function parseRemoteGameCatalogHtml(html: string): ParsedRemoteTrainer[] {
  ...
  while ((match = regex.exec(html)) !== null) {
    const href = match[1];
    const gameName = decodeHtmlEntities(match[2].trim());
    if (!gameName || gameName.length < 2 || isPlaceholderTitle(gameName)) continue;
    const key = gameName.toLowerCase();
    if (seen.has(key)) continue;
    seen.add(key);
