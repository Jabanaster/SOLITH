...
export interface ProcessPickerOption {
  ...
  matchedBy: 'current-game' | 'installed-executable' | 'catalog-executable' | 'game-like-name' | 'unknown';
...
export function buildProcessPickerOptions(input: BuildProcessPickerOptionsInput): ProcessPickerOption[] {
  ...
  processes.forEach((process, index) => {
    ...
    const installedGame = installedByExecutable.get(processName) ?? null;
    ...
    if (currentMatch && currentGame) {
      ...
      option = {
        ...
        matchedBy: 'installed-executable',
        detectedOrdinal: index,
      };
    } else if (catalogGame) {
      option = {
