...
## 6. Local build, sideload, launch, and test

The project uses an ignored, local-only, self-signed code-signing certificate.
Create or rotate it from PowerShell before the first build:

```powershell
cd G:\ACTIVE_PROJECTS\SOLITH-GAMEBAR\gamebar-widget-poc\WispGameBarWidget
.\New-LocalDevCertificate.ps1
```

The private PFX and public CER are intentionally excluded from Git. The script
uses the manifest publisher `CN=SolithLocalDevPoc`, and MSBuild derives the
thumbprint from the PFX so each developer can use a different local key.

Build and package x64 Debug with Visual Studio 2022 MSBuild:

```powershell
& 'D:\Visual Studio\MSBuild\Current\Bin\MSBuild.exe' `
  .\WispGameBarWidget.csproj /restore `
...
```text
AppPackages\WispGameBarWidget_0.1.0.0_x64_Debug_Test\
```

Windows AppX deployment requires this self-signed development certificate in
the machine `TrustedPeople` store; `CurrentUser\TrustedPeople` is not used for
package signature trust. In an administrator PowerShell window:

```powershell
$package = Resolve-Path '.\AppPackages\WispGameBarWidget_0.1.0.0_x64_Debug_Test'
Import-Certificate `
  -FilePath '.\WispGameBarWidget_LocalDevKey.cer' `
  -CertStoreLocation 'Cert:\LocalMachine\TrustedPeople'
& "$package\Add-AppDevPackage.ps1" -Force -SkipLoggingTelemetry
```

This changes machine trust and remains a manual administrator gate. Never
import the private PFX into a trust store. If Windows reports that sideloading
is disabled, enable Developer Mode under Settings -> Privacy & security -> For
developers, then repeat the install.

After installation, complete the manual host gate:

1. Open Game Bar (`Win+G`) and confirm the widget appears and can be pinned.
   **Passed 2026-07-29:** the widget opened and remained visible after Game
   Bar closed while pinned.
2. Launch a real game (windowed, borderless, and fullscreen) and verify: the
   ...
   hides the button as coded, alt-tab, and minimize/restore. **Pending.**
3. Run `python mock-solith-service/server.py` and confirm the widget's "Ping
   ...
   its own in the original sandbox pass). **Passed 2026-07-29:** the widget
   displayed the mock service's JSON response with `"status": "ok"`.

Runtime inspection after the manual pass found one responsive
`WispGameBarWidget` process, package status `Ok`, and no matching Windows
Application error or critical events in the inspection window. Repeated
open/close duplicate-instance testing and explicit unpin behavior remain
pending.
...
- **Local test signature only.** `Package.appxmanifest`'s
  `Identity/Publisher` is the placeholder `CN=SolithLocalDevPoc`. The generated
  package is signed, but only with a per-developer self-signed certificate. It
  is not production-signed or distributable as a trusted release.
